...
describe('TapTrack NFC v0.2.0 ALPHA — Comprehensive Test Suite', () => {
  ...
  describe('Change Password', () => {
    it('changes password, reissues the current cookie, and revokes the previous session', async () => {
      ...
      usersTable.push({
        id: 1,
        email: 'password.user@yopmail.com',
        password_hash: hash,
        first_name: 'Password',
        last_name: 'User',
        role: 'USER',
      ...
      const loginRes = await request(app)
        .post('/api/auth/login')
        .send({ email: 'password.user@yopmail.com', password: originalPassword });
      ...
      expect(wrongCurrent.body.error).toMatch(/current password is incorrect/i);
      ...
      expect(changed.body.user.password_hash).toBeUndefined();
      expect(changed.body.message).toMatch(/password changed successfully/i);
      expect(await bcrypt.compare(newPassword, usersTable[0].password_hash)).toBe(true);

      const replacementCookie = changed.headers['set-cookie'];
      ...
      expect(oldSession.body.code).toBe('SESSION_REVOKED');

      const currentSession = await request(app)
        .get('/api/auth/me')
        .set('Cookie', replacementCookie);
      expect(currentSession.status).toBe(200);
      expect(currentSession.body.user.email).toBe('password.user@yopmail.com');
    });
  });
