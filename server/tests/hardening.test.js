...
const participantRepo = vi.hoisted(() => ({ getRequiredAttendanceStats: vi.fn() }));
vi.mock('../src/repositories/eventParticipants.repository.js', () => ({ eventParticipantsRepository: participantRepo }));
...
describe('v0.8 activity pulse and audit hardening', () => {
  it('calculates total check-ins and distinct events', () => {
    ...
    expect(result).toMatchObject({ totalCheckIns: 3, eventsAttended: 2, attendanceRate: null, attendanceRateLabel: 'No required events yet', currentStreak: 2 });
  });
  ...
  it('loads pulse inputs only for the requested user', async () => {
    attendanceRepo.listByUser.mockResolvedValue([]);
    participantRepo.getRequiredAttendanceStats.mockResolvedValue({ eligibleEvents: 0, attendedEvents: 0 });
    await activityPulseService.getForUser(42);
    expect(attendanceRepo.listByUser).toHaveBeenCalledWith(42);
    expect(participantRepo.getRequiredAttendanceStats).toHaveBeenCalledWith(42);
  });
